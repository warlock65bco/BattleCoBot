...
async def on_reaction_add(reaction, user):
    ...
    if str(user) != "Oxyg3n's WS and RS Organizer#7677" and str(reaction.message.author) == "Oxyg3n's WS and RS Organizer#7677":
        ...
        if(rs_queue != 0 and rs_queue != -1): # Add person to respective queue
            load_dotenv()
            api_request_str = os.getenv('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI3MjQ5Mjk5OTQ1NTk1ODYzNjQiLCJndWlsZElkIjoiMzkzMTQ5NjIxNDYwOTI2NDY0IiwiaWF0IjoxNjIyNTEyNzA0LCJleHAiOjE2NTQwNzAzMDQsInN1YiI6ImFwaSJ9.bp_HyygeL5CmRz-asaUXMP6rdoXp374lHTjRb-XPjHg') + str(user.id)
...
@bot.command(help="Check if bot is alive")
async def ping(ctx):
	await ctx.send('I live!')
