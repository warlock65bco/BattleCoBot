...
API_KEY_REQUEST = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI3MjQ5Mjk5OTQ1NTk1ODYzNjQiLCJndWlsZElkIjoiMzkzMTQ5NjIxNDYwOTI2NDY0IiwiaWF0IjoxNjIyNTEyNzA0LCJleHAiOjE2NTQwNzAzMDQsInN1YiI6ImFwaSJ9.bp_HyygeL5CmRz-asaUXMP6rdoXp374lHTjRb-XPjHg"

#URL = https://bot.hs-compendium.com/compendium/api/tech?token
...
class BattleCoWSCogs(commands.Cog, name='BattleCo'):
    ...
    async def check_people(self):
        print("Checking for people in the RS queue")
    ...
    async def rs(self, ctx, level=None):
        if(level is None): # Join their RS Scanners level queue
            ...
            load_dotenv()
            #temp = str(os.getenv('API_KEY_REQUEST'))
            #await ctx.send('Test!')
            #await ctx.send(temp)
            #api_request_str = os.getenv('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI3MjQ5Mjk5OTQ1NTk1ODYzNjQiLCJndWlsZElkIjoiMzkzMTQ5NjIxNDYwOTI2NDY0IiwiaWF0IjoxNjIyNTEyNzA0LCJleHAiOjE2NTQwNzAzMDQsInN1YiI6ImFwaSJ9.bp_HyygeL5CmRz-asaUXMP6rdoXp374lHTjRb-XPjHg') + str(user_id)
            api_request_str = os.getenv('API_KEY_REQUEST') + str(user_id)
	#os.getenv is returning NONE here ----- AJ 05.31.2021
            #print("Test:",os.getenv('API_KEY_REQUEST'))
            ...
            user_id = ctx.author.id
            load_dotenv()
            api_request_str = str(os.getenv('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI3MjQ5Mjk5OTQ1NTk1ODYzNjQiLCJndWlsZElkIjoiMzkzMTQ5NjIxNDYwOTI2NDY0IiwiaWF0IjoxNjIyNTEyNzA0LCJleHAiOjE2NTQwNzAzMDQsInN1YiI6ImFwaSJ9.bp_HyygeL5CmRz-asaUXMP6rdoXp374lHTjRb-XPjHg')) + str(user_id)
            api_request = requests.get(api_request_str)
            total_info = api_request.json()
            rs_level = total_info["map"]["rs"]["level"]
